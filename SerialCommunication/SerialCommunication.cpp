...
void SerialCommunication::sendData(byte address,char getSet,byte sensor,byte firstSensorValue,byte secondSensorValue){
    if (_useRS485) digitalWrite(_RS485resvPin, RS485Transmit);
    _SoftSerial.write('!');
    _SoftSerial.write(address);
    _SoftSerial.write(getSet);
    _SoftSerial.write(sensor);
    _SoftSerial.write(firstSensorValue);
    _SoftSerial.write(secondSensorValue);
    _SoftSerial.write(_ownAddress);
    _SoftSerial.write('#');
    if (_useRS485)digitalWrite(_RS485resvPin, RS485Receive);
}
...
resvData.destAddress=_receivedBytes[0];
